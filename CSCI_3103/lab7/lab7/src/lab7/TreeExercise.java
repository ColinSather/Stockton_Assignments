...
public class TreeExercise {
	 ...
	 *                                      a   b       
	 *
	 * Post order: return: "x y + a b + c - *"
	 *
	 * Pre order: return: "* + x y / + a b c"
"
	 */
	...
	public static String inOrderTraveral(Node<String> root)
	{
		if (root == null)
		 ...
		 return inOrderTraveral(root.left) + root.data + inOrderTraveral(root.right); // remove this line
	}
	...
	public static String levelOrderTraveral(Node<String> root)
	{
		...
		Queue<Node<String>> asdf = new LinkedList<Node<String>>();
		que.add(root);
		while(!que.isEmpty()) {
...
/* ====== Extra Credit (10%)  ==============================*/
	...
	{
		Queue<Node<String>> que = new LinkedList<Node<String>>();
		ArrayList<String> lvlOrder = new ArrayList<String>();
		lvlOrder.add(root.toString());
		que.add(root);
		while (!que.isEmpty()) {
			Node<String> current = que.poll();
			if (current.left != null) {
				lvlOrder.add(current.left.toString());
				que.add(current.left);
			}
			if (current.right != null) {
				lvlOrder.add(current.right.toString());
				que.add(current.right);
			}
		}
		return lvlOrder;
