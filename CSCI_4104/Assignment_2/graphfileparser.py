...
def haversine(lat1, lng1, lat2, lng2) :
    ...
    delata_lambda = math.radians(lng2-lng1)

    a = math.sin(delta_phi/2) * math.sin(delta_phi/2) + math.cos(phi_1) * math.cos(phi_2) * math.sin(delata_lambda/2) * math.sin(delata_lambda/2)
...
def parseHighwayGraphFile(filename) :
    ...
    with open(filename, 'r') as f:
        ...
        V, E = all_lines[1].split()
        V, E = int(V), int(E)
        section = len(all_lines) - E

        # format highway data from Andorra region
        for line in range(len(all_lines)):
            temp = all_lines[line].split()
            if 1 < line < section:
                temp.pop(0)
                ...
                for x in temp:
                    [temp.pop() for i in temp if len(temp) > 2]
                temp = [int(i) for i in temp]
                edges.append(temp)
        f.closed

    # add weighted edges to adjacency matrix
    matrix = WeightedGraph(V)
    for edge in edges:
        corda = verticies[edge[0]]
        cordb = verticies[edge[1]]
        w = haversine(corda[0], corda[1], cordb[0], cordb[1])
        matrix.addEdge(edge[0], edge[1], w)
        matrix.addEdge(edge[1], edge[0], w)
    return matrix


if __name__ == "__main__" :
    # Note: set command line argument equal to the path of AND-region.txt when running module
    # 3a) get input file command line argument
    print("\nThe file we are parsing is", sys.argv[1])
    
    # 3b) call your parseHighwayGraphFile function to parse that file and create graph object
    graph = parseHighwayGraphFile(sys.argv[1])
    
    # 3c) get the degree of each vertex
    print("\nThe degree of each vertex is in the format (vertex, degree).")
    for v in range(graph.numVertices()):                
        print(v, graph.degree(v))
    
    # 3d) Haversine test section
    print("\nTesting haversine function with cordinates 50, 5, 58, 3")
    print(haversine(50, 5, 58, 3))
